					   ...
					   const std::string &label_file)
{
	...
	{
		labels_.emplace_back(std::string(line));
	}
...
static bool compare_pair(const std::pair<float, int> &lhs, const std::pair<float, int> &rhs)
{
	return lhs.first > rhs.first;
}

// Return the indices of the top N values of vector v
static std::vector<int> get_high_vals(const std::vector<float> &v, int N)
{
	std::vector<std::pair<float, int>> pairs;

	for (size_t i = 0; i < v.size(); ++i)
	{
		pairs.emplace_back(std::make_pair(v[i], i));
	}

	std::partial_sort(pairs.begin(), pairs.begin() + N, pairs.end(), compare_pair);
...
std::vector<Prediction> Classifier::classify(const cv::Mat &img, int N)
{
	...
	std::vector<int> maxes = get_high_vals(output, N);
	std::vector<Prediction> predictions;

	for (int i = 0; i < N; ++i)
	{
		int idx = maxes[i];
		predictions.emplace_back(std::make_pair(labels_[idx], output[idx]));
	}
...
// Wrap the input layer of the network in separate cv::Mat objects (one per channel). This way
// we save one memcpy operation and we don't need to rely on cudaMemcpy2D. The last preprocessing
...
void Classifier::preprocess(const cv::Mat &img, std::vector<cv::Mat> *input_channels)
{
	...
	// This operation will write the separate BGR planes directly to the input layer of the
	// network because it is wrapped by the cv::Mat objects in input_channels
